...
new_code = """    def _build(self):
        ...
        ctk.CTkLabel(self, text=title_txt, font=FONTS["heading"], text_color=COLORS["text_primary"]).pack(anchor="w", pady=(0, 12))
        ...
        ctk.CTkLabel(t_notes, text="Notas adicionales", font=FONTS["label"], text_color=COLORS["text_muted"]).pack(anchor="w", padx=8, pady=(8, 2))
        self._entries["notes"] = ctk.CTkTextbox(t_notes, height=80, fg_color=COLORS["bg_input"], border_color=COLORS["border"])
        self._entries["notes"].pack(fill="x", padx=8, pady=(0, 8))
        ...
        if ext.equipment_id and hasattr(self, "_equipment_combo"):
            for opt in self._equipment_combo.cget("values"):
                if opt.startswith(f"{ext.equipment_id}:"):
                    ...
                    break
"""
content = content.replace(populate_pattern, populate_new)
